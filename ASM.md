...
```
## SSE
Жирное расширение, чтобы закопать 3dnow. Однако продумано оно было меньше (например для комплексных чисел). SSE2 - добавили xmm регистры. C переходом на 64 бита - удвоили кол-во регистров (стало 16).

SSSE3 - добавили 
pshufb (переставляет байты (в каждом байтике сказано откуда взять числа)). 
aligner - выравнивал данные

SSE4 - заврыли всякие дырки. Добавили 6 целочисленных команд. Добавили popcount. 

SSE4A - позваляет выключить падение от невыравненности.

AWX - xmm раширили до ymm и c ними можно работать только как с плавающей точкой.
AWX2 - разрешили использовать целочисленные операции.

AWx512 - расширили регистры до 512 (zmm) и удвоили кол-во регистров. Добавили маски (можно выключать некоторые части регистров).

## Режимы работы процессора
Как имея 16 битные регистры обратиться к мегабайту памяти? Сегменты :( . 

ТО что мы используем в квадратных скобках - это оффсет. seg:offset. 16 битные регистры:
```
cs
ds
ss
...
адрес = seg * 16 + offset. Можно выйти за мегабайт, в таком случае возвращаемся в начало. 

Позже в 286ом появился протекткд мод. Регитср превратился в index|l/g(1бит)|rpl(2бита)

rpl - 4 стостояния (3 - юзер спейс, 0 - root, промежуточные просто сущестуют :( )

l/g - local or global table. В современном мире используется только global table.

### 386ой
Он уже стал 32ух битным (вариант защищенного режима, где все команды интерпретируются как 32ух битные). 
За счет префикса 066 можно было переключиться с 16 на 32 и обратно. 

Добавили V86. Обращение как в реальном, но код работает в режиме 3го кольца. Но устройства эмулирует операционка.

### Страничная адресация
Вся память разбивается на кусочки по размеру страницы (обычно 4кб). 0вую и последнюю страницы виртуальной памяти 
обычно не выделяют, чтобы nullptr падал.
